---
type: "Workflow"
title: "Budget and Financial Forecast source-aware workflow"
description: "Verify-first workflow for producing a reviewable budget and financial forecast package."
---
# Source-Aware Workflow

1. Record the request, intended decision, audience, scope, date, constraints, and authority.
2. Verify current standards, originator guidance, required sections, evidence expectations, audience needs, and local approval procedures.
3. Inventory approved historical actuals, chart of accounts, entity and period, revenue and cost drivers, staffing and contract plans, tax and financing assumptions, cash timing, scenario definitions, dependencies, reconciliations, model controls, sensitivity, and approvals.
4. Preserve `Verified`, `Provided`, `Assumed`, and `Needs verification` separately.
5. Define included and excluded work, records, systems, permissions, dependencies, validation, and rollback or stop conditions.
6. Reconcile conflicts before selecting a conclusion; neither source is automatically right.
7. Draft the budget and financial forecast package, including alternatives, risks, dependencies, owners only when evidenced, review points, and stop conditions.
8. Require explicit confirmation before post entries, commit revenue or cost, hire, contract, borrow, invest, spend, publish guidance, or represent forecast certainty.

## Required Output

### Direct Answer
State what can and cannot be concluded.

### Evidence Status
- Verified:
- Provided:
- Assumed:
- Needs verification:

### Verification Plan
List exact sources, records, checks, conflicts, and reviewers.

### Confirmation Boundary
Name the evidenced authorized reviewer and prohibited actions.

### Source Note
List authoritative sources used, local evidence used, and missing sources.
