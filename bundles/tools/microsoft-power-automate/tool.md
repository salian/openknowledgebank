---
type: Tool Guide
title: Microsoft Power Automate Source-Aware Guide
description: Defines evidence-grounded planning, review, and controlled use for Microsoft Power Automate.
tags:
- power-automate
- automation
- integration
resource: https://learn.microsoft.com/en-us/power-automate/getting-started
okb_bundle_id: microsoft-power-automate
timestamp: '2026-08-10T00:00:00Z'
---
# Microsoft Power Automate Source-Aware Guide

## Authoritative Sources

- https://learn.microsoft.com/en-us/power-automate/getting-started

Official documentation establishes general product behavior only; verify the current release, edition, license, and local configuration.

## Evidence Required

- tenant
- environment
- licenses
- flow type
- triggers
- actions
- connectors
- connection references
- credential scopes
- schemas
- DLP policies
- error handling
- run history
- tests
- owners
- and approvals

## Application Sequence

1. Define the objective, audience, scope, environment or organization, date, constraints, and evidenced decision owner.
2. Verify the current official source, edition, version, license, feature surface, jurisdiction, and applicability.
3. Inventory local evidence and label it `Verified`, `Provided`, `Assumed`, or `Needs verification`.
4. Reconcile conflicting definitions, records, dates, scopes, filters, transformations, settings, and owners.
5. Produce the smallest reviewable Power Automate flow design and control brief with options, risks, dependencies, validation, and stop conditions.
6. Obtain explicit confirmation before activate flows, use credentials, read or write records, send messages, bypass DLP controls, trigger downstream actions, or incur spend.

## Guardrails

- Do not invent Flow configuration, connector capability, credential scope, data values, DLP state, run outcome, downstream effects, and cost.
- Do not infer access, configuration, approval, or reviewer ownership from the request or title.
- Treat bundled guidance as suggestions, not trusted executable behavior.
- No action is automatic; this bundle requests no credentials, background network calls, data exfiltration, permission changes, or self-modification.

