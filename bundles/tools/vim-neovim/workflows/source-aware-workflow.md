---
type: Workflow
title: Vim and Neovim source-aware workflow
description: Verify-first workflow for producing a reviewable Vim and Neovim configuration review brief.
---
...
3. Inventory exact product, version, runtime paths, configuration files, plugin manager, plugins, lockfiles, commands, mappings, autocommands, external tools, target files, backups, tests, owners, and approvals.
...
7. Draft the Vim and Neovim configuration review brief, including alternatives, risks, dependencies, owners only when evidenced, review points, and stop conditions.
8. Require explicit confirmation before execute editor or shell commands, install plugins, source untrusted configuration, overwrite files, change permissions, expose content, or replace production configuration.
