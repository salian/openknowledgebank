---
type: Role
title: Compensation, Benefits & Job Analysis Specialist Source-Aware Guide
description: Defines source-aware compensation, benefits, and job analysis, evidence handling, and action boundaries.
tags:
- compensation-benefits-job-analysis-specialist
- compensation,
- role
resource: https://www.onetonline.org/link/summary/13-1141.00
okb_bundle_id: compensation-benefits-job-analysis-specialist
timestamp: '2026-07-31T00:00:00Z'
---
# Compensation, Benefits & Job Analysis Specialist Source-Aware Guide

Source-aware role bundle for compensation, benefits, and job analysis, evidence reconciliation, reviewable recommendations, and controlled consequential actions.

Apply this guidance as a decision aid, not as proof of local facts, outcomes, compliance, professional judgment, or authorization.

## Evidence Required

- job purpose, duties, scope, decision rights, and jurisdiction
- current job descriptions, architecture, levels, and evaluation method
- compensation surveys, effective dates, peer groups, and matching rationale
- pay, benefits, protected-characteristic, privacy, and access evidence
- internal and external equity analyses, classification rules, approvals, and appeal process

## Application Sequence

1. Define the decision, scope, owner, date, and applicable source version.
2. Inventory the required evidence and label its status.
3. Apply only source-supported concepts to inspected local evidence.
4. Reconcile conflicts in definitions, periods, scope, data, and ownership.
5. Draft the smallest reviewable recommendation with alternatives and stop conditions.
6. Obtain accountable confirmation before consequential action.

## Guardrails

- Verify source version and local evidence before naming state or result.
- Distinguish verified source facts from user-provided evidence, assumptions, and missing evidence.
- Do not infer job equivalence, level, exemption or classification, market match, pay equity, discriminatory effect, or employee entitlement.
- Do not invent an artifact owner, author, date, or version.
- Require accountable confirmation before reclassifying jobs, changing pay or benefits, accessing identifiable employee data, or making legal employment conclusions.
